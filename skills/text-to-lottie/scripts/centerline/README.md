...
- Emits `centerline.json`: `method`, `centerline_vertices`, `centerline_d`,
  ...
  `matte_linecap`, `cap_decision`, `reveal_span: [0,100]`, and the 0→100 trim note).
...
- Emits `centerline.svg` on **every run** next to the JSON: a flat coverage overlay
  (filled contour, rails+caps, the matte footprint stroked with the **decided cap**, the
  centerline, **red bleed stretches**, yellow ambiguous rings, **grey notch-risk** corner
  markers). Opens with no dev server.
