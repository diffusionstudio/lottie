...
| Single path, centerline, or one-stroke driver from a filled SVG/logo/icon; path-driven mask; path-driven reveal; shape-following mask; shape-following reveal; stroke matte or path-revealed matte; Trim Paths reveal from a filled mark; or mask driver path | `references/single-line-vectorization.md`, plus the task recipe and `references/svg-compatibility.md` |
| Redraw / draw-on / write-on where the **stroke is the visible art**: handwriting, handwritten text animation, signature, "looks hand-drawn", write-on / draw-on, an animated single continuous pen line drawing itself on (no fill hidden underneath) | `references/single-line-redraw.md`, plus the task recipe and `references/svg-compatibility.md` |
...
4. Write or update `public/projects/<project>/<scene-N>/lottie.json` and, when
   ...
   source is a nontrivial filled mark, perform the geometry sandbox pass before
